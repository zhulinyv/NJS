...
class ConfigModel(BaseModel):
    ...
    sim_gacha_cd_group: int = Field(3, alias='模拟抽卡群冷却')
    sim_gacha_cd_member: int = Field(90, alias='模拟抽卡群员冷却')
    sim_gacha_max: int = Field(9, alias='模拟抽卡单次最多十连数')
    ...
    auto_sign_hour: int = Field(7, alias='米游社签到开始时间(小时)')
    auto_sign_minute: int = Field(0, alias='米游社签到开始时间(分钟)')
    ...
    guess_voice_time: int = Field(60, alias='原神猜语音时间')
