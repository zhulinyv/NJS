...
async def wb_set(
    ...
    try:
        ...
        # 替换掉莫名出现的 amp; 来解决
        wb.set(index, type_, Message(key.replace("amp;", '')), str(value).replace("amp;", ''), require_to_me)
...
async def wb_search(
    ...
    if isinstance(event, GroupMessageEvent):
        ...
        for entry in entrys:
            ...
            for value in entry.values:
                msg_temp += "\n" + Message.template("{value}").format(value=str(value).replace("amp;", ''))
