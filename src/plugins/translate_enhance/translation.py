...
async def translate(from_, text: str, to: str):
    # en,jp,zh
    for i in range(config.novelai_retry):
        try:
            result = (await translate_deepl(from_, text, to) or 
                      await translate_bing(from_, text, to) or
                      await translate_baidu(from_, text, to) or
                      await translate_youdao(from_, text, to) or
                      await translate_google_proxy(from_, text, to)
                      )
        except:
...
async def translate_bing(from_: str, text: str, to: str):
    """
    en,jp,zh_Hans
    """
    if to == "zh":
...
async def translate_deepl(from_: str, text: str, to: str):
    """
    EN,JA,ZH
    """
    to = to.upper()
...
async def translate_youdao(from_: str, input: str, type_: str):
    """
    默认auto
    ZH_CH2EN 中译英
    EN2ZH_CN 英译汉
    """
    if from_ == "zh" and type_ == "en":
        type = "ZH_CH2EN"
    elif from_ == "en" and type_ == "zh":
        type = "EN2ZH_CN"
    else:
        return None
    async with aiohttp.ClientSession() as session:
...
async def translate_google_proxy(from_: str, input: str, to: str):
    """
    en,jp,zh 需要来源语言
    """
    async with aiohttp.ClientSession()as session:
...
async def translate_baidu(from_: str, input: str, to: str):
